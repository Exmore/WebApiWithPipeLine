...
{
    public class SeparateWordsResult : IResultContainer<SeparateWordsOutModel>
    {
        private readonly List<ApplicationException> _appErrors;

        public SeparateWordsResult(SeparateWordsOutModel result)
        {
            _appErrors = new List<ApplicationException>();
            Value = result;
        }
        public SeparateWordsOutModel Value { get; }
