...
namespace PipeLine.Domain.Steps.AddInfoToFile
{
    public class SecondStep : IStep<SecondStepInModel, SecondStepResult>
    {
        ...
        public SecondStep(string filePath)
        {
            _filePath = filePath;
        }
        ...
        {
            //Main Action
            File.AppendAllText(_filePath, $"Second Step Start of the {secondStepInModel} at {DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss")}\n");
            Thread.Sleep(secondStepInModel.DelayTime * 1000);
            File.AppendAllText(_filePath, $"Second Step of the {secondStepInModel} has been done at {DateTime.Now.ToString("dd.MM.yyyy hh:mm:ss")}\n");            
